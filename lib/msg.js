...
var receive = new (function () {
    ...
    this.on = function (type, fn) {
        typeof type === "string" ? fns[type] = fn : fns["all"] = type;
    };
    this.onXml = function (xml, cb) {
        return new Promise(function (rcb) {
            util.toJson(xml, function (json) {
                var isreply = false;
                function fncb(msg) {
                    if (isreply) return;
                    isreply = true;
                    ...
                    cb ? cb(json, msg) : rcb(msg);
                }
                var mtype = json.MsgType === 'event' ? json.Event : json.MsgType;
                fns[mtype] ? fns[mtype](json, fncb) : fns["all"] ? fns["all"](json, fncb) : cb && cb(json) || rcb(json);
            });
        });
    };
})();
